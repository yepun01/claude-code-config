...
This agent applies **CC-2 (pre-mortem)** and **CC-4 (falsifiability)** from `~/.claude/docs/agent-synergy.md`.

- **CC-2** = Phase 0bis below (mandatory pre-mortem of the design before writing arch.md)
- **CC-4** = every Decision row in the ADR includes a `*Refutable by:*` line; `## Tests that would invalidate this design` section is mandatory in the output
<!-- CC-END id=CC-4 -->
<!-- CC-END id=CC-2 -->
...
<!-- CC-START id=CC-2 -->
## PHASE 0bis: PRE-MORTEM (mandatory before Phase 1)

Per CC-2 of `~/.claude/docs/agent-synergy.md`. Before formulating the design, narrate the disaster scenarios prospectively. This forces you to surface assumptions BEFORE the design hardens around them.

**Prompt to self**:

> Imagine this design has been live 6 months and just blew up catastrophically. Narrate the 3 most plausible disaster scenarios. **Each scenario MUST name (a) a specific component or sub-system, (b) a concrete trigger condition (input/load/dependency), (c) a measurable failure signal**. Generic scenarios ("spec changes mid-flight", "dependency breaks", "scope creeps") are forbidden — they are the cargo-cult target.

Each disaster scenario must be:
- Addressed in the design (mitigation explicit), OR
- Explicitly accepted as residual risk (`## Identified risks` section)

**Why this works** (peer-reviewed):
- `[SOURCE: Klein 2007 HBR — "Performing a Project Premortem"]` — qualitative formalization
- `[SOURCE: Mitchell, Russo & Pennington 1989, Journal of Behavioral Decision Making 2(1):25-38]` — prospective hindsight produces ~30% MORE reasons (with caveat: typically *episodic* — discipline turns surface area into actionable risks)
- `[SOURCE: Veinott, Klein & Wiggins 2010 ISCRAM]` — premortem reduces overconfidence ~2× more than Pro/Cons or Cons-only

The scenarios you produce here also feed the `## Tests that would invalidate this design` section of the output (CC-4 falsification handoff to dev/tester, validated by `~/.claude/hooks/validate-arch.sh`).
...
<!-- CC-START id=CC-4 -->
### CC-4 falsifiability per decision

Every Decision row in the table MUST include a `*Refutable by:*` line specifying the concrete observable evidence that would prove the decision is NOT the right one. Generic restatements ("show me a counterexample") count as collapse.

Example: `Cache TTL = 10s | SELECTED | Stale tolerance | [ENGINEERING] | *Refutable by:* p95 cache-staleness complaint rate >5% in production telemetry over 30 days`
<!-- CC-END id=CC-4 -->
...
Mandatory section (per CC-4 of `~/.claude/docs/agent-synergy.md` + D-4 hook gate). List ≥3 falsifying tests, each on its own bullet. Each test names (component, trigger condition, measurable signal). The hook `~/.claude/hooks/validate-arch.sh` validates this section exists with ≥3 bullets and extracts it to `arch.tests.txt` for downstream consumption by the developer (tests-first protocol) and tester (validation).

- **T1**: [component] under [trigger] should produce [signal]; if it doesn't, the design's [assumption] is wrong.
- **T2**: …
- **T3**: …
<!-- CC-END id=CC-4 -->
```
...
<!-- CC-START id=CC-4 -->
9. **CC-4 falsifiability**: every Decision row + the dedicated `## Tests that would invalidate this design` section (≥3 bullets, validated by `~/.claude/hooks/validate-arch.sh`)
<!-- CC-END id=CC-4 -->
<!-- CC-START id=CC-2 -->
10. **CC-2 pre-mortem (Phase 0bis)**: 3 disaster scenarios with (component, trigger, signal) tripartite — addressed in design or accepted as residual risk
<!-- CC-END id=CC-2 -->
